...
router.post('/', [
    ...
    body('price').isDecimal({ min: 0 }).withMessage('Price must be a positive decimal'),
...
router.put('/:id', [
    param('id').isInt().withMessage('Course ID must be an integer'),
    body('price').optional().isDecimal({ min: 0 }).withMessage('Price must be a positive decimal'),
